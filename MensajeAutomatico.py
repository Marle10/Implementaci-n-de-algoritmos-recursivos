...
def buscar_mensaje(conversacion, mensaje_buscar, indice=0):

    # Caso base: si el índice supera el límite de la conversación, el mensaje no se encontró
    if indice >= len(conversacion):
        return -1
    
    # Si el mensaje en el índice actual coincide con el mensaje buscado, se devuelve el índice
    if conversacion[indice] == mensaje_buscar:
        return indice
    
    # Llamada recursiva para buscar el mensaje en el resto de la conversación
    return buscar_mensaje(conversacion, mensaje_buscar, indice + 1)
...
mensaje_buscado = "¿Quieres salir hoy?"
indice_encontrado = buscar_mensaje(conversacion_whatsapp, mensaje_buscado)

if indice_encontrado != -1:
    print(f"El mensaje '{mensaje_buscado}' se encuentra en el índice {indice_encontrado} de la conversación.")
else:
    print(f"El mensaje '{mensaje_buscado}' no se encontró en la conversación.")
